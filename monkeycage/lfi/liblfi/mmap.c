...
static bool ispow2(uint64_t x) { return (x & (x - 1)) == 0; }

static size_t getpow(uint64_t x) {
  size_t r = 0;
  while (x >>= 1) {
    r++;
  }
...
static bool mmvalid(struct MMAddrSpace* mm, uint64_t start, size_t len) {
  return start >= mm->base && (start + len) <= (mm->base + mm->len);
}

static uint64_t mmtrunc(struct MMAddrSpace* mm, uint64_t addr) {
  return addr >> mm->p2pagesize;
}

static uint64_t mmceil(struct MMAddrSpace* mm, uint64_t addr) {
  uint64_t align = 1 << mm->p2pagesize;
  uint64_t align_mask = align - 1;
  return ((addr + align_mask) & ~align_mask) >> mm->p2pagesize;
}

static bool contained(uint64_t key1, uint64_t size1, uint64_t key2,
                      uint64_t size2) {
  return key1 >= key2 && key1 + size1 <= key2 + size2;
}

static void node_insert_before(struct MMAddrSpace* mm, struct MMNode* n,
                               struct MMNode* new) {
  assert(n);
  new->next = n;
  new->prev = n->prev;
...
static void node_insert_after(struct MMAddrSpace* mm, struct MMNode* n,
                              struct MMNode* new) {
  (void)mm;
  assert(n);
  new->next = n->next;
...
static void node_remove(struct MMAddrSpace* mm, struct MMNode* n) {
  if (n->next) {
    n->next->prev = n->prev;
  }
  if (n->prev) {
...
bool mm_init(struct MMAddrSpace* mm, uintptr_t start, size_t len,
             size_t pagesize) {
...
void mm_free(struct MMAddrSpace* mm) {
...
void mm_mark_original(struct MMAddrSpace* mm) {
...
static uintptr_t mm_mapany_cursor(struct MMAddrSpace* mm, size_t len, int prot,
...
uintptr_t mm_mapany(struct MMAddrSpace* mm, size_t len, int prot, int flags,
                    int fd, off_t offset) {
  uintptr_t p = -1;
  if (mm->cursor)
    p = mm_mapany_cursor(mm, len, prot, flags, fd, offset, mm->cursor);
  if (p != (uintptr_t)-1) return p;
  return mm_mapany_cursor(mm, len, prot, flags, fd, offset, mm->nodes);
}

uintptr_t mm_mapat(struct MMAddrSpace* mm, uintptr_t addr, size_t length,
                   int prot, int flags, int fd, off_t offset) {
  return mm_mapat_cb(mm, addr, length, prot, flags, fd, offset, NULL, NULL);
}

uintptr_t mm_mapat_cb(struct MMAddrSpace* mm, uintptr_t addr, size_t length,
                      int prot, int flags, int fd, off_t offset, UpdateFn ufn,
                      void* udata) {
  if (addr % (1 << mm->p2pagesize) != 0) return (uintptr_t)-1;
  addr = mmtrunc(mm, addr);
  length = mmceil(mm, length);

  if (!mmvalid(mm, addr, length)) return -LINUX_EINVAL;

  struct MMNode* new = malloc(sizeof(struct MMNode));
  if (!new) return (uintptr_t)-1;

  int r = mm_unmap_cb(mm, addr << mm->p2pagesize, length << mm->p2pagesize, ufn,
                      udata);
  assert(r == 0);
  ...
  *new = (struct MMNode){
  ...
  struct MMNode* node = mm->nodes;
  if (mm->cursor && mm->cursor->base <= addr) node = mm->cursor;
  if (!node) {
    mm->nodes = new;
    ...
    while (node) {
      ...
      if (addr >= node->base + node->len && addr + length <= next) break;
      node = node->next;
    }
    assert(node);
...
int mm_unmap(struct MMAddrSpace* mm, uintptr_t addr, size_t length) {
  return mm_unmap_cb(mm, addr, length, NULL, NULL);
}

int mm_unmap_cb(struct MMAddrSpace* mm, uintptr_t addr, size_t length,
                UpdateFn ufn, void* udata) {
  if (addr % (1 << mm->p2pagesize) != 0 || length == 0) return -LINUX_EINVAL;
  addr = mmtrunc(mm, addr);
  length = mmceil(mm, length);
  ...
  while (node) {
    if (contained(node->base, node->len, addr, length)) {
      ...
      if (mm->cursor == cursor) mm->cursor = node->prev;
      node = node->next;
      ...
      if (ufn)
        ufn(addr << mm->p2pagesize, length << mm->p2pagesize, node->info,
            udata);
      *new = (struct MMNode){
          .base = addr + length,
      ...
      if (ufn)
        ufn(addr << mm->p2pagesize, (node_end - addr) << mm->p2pagesize,
            node->info, udata);
      if (mm->cursor == cursor) mm->cursor = node;
      node->len = addr - node->base;
      node = node->next;
      ...
      if (ufn)
        ufn(node->base << mm->p2pagesize,
            (addr + length - node->base) << mm->p2pagesize, node->info, udata);
      size_t node_end = node->base + node->len;
  ...
  if (!used_new) free(new);

  return 0;
}

void mm_unmap_non_original(struct MMAddrSpace* mm, UpdateFn ufn, void* udata) {
  struct MMNode* node = mm->nodes;
  while (node) {
    if (!node->info.original)
      mm_unmap_cb(mm, node->base << mm->p2pagesize, node->len << mm->p2pagesize,
                  ufn, udata);
    node = node->next;
  }
}
