...
#include "mcapi.h"  // MC::RootedValueArray
#include "mcfriendapi.h"
#include "js/ArrayBuffer.h"  // JS::Is{,Detached}ArrayBufferObject
#include "js/GCPolicyAPI.h"
#include "js/JSON.h"
#include "monkeycage/RootingAPI.h"  // JS::{Handle,Heap},PersistentRooted
...
class EventRunnable final : public MainThreadProxyRunnable {
  ...
  MC::PersistentRooted<JSObject*> mScopeObj;
...
Proxy::HandleEvent(Event* aEvent) {
...

  {
    ...
    MC::Rooted<JSObject*> scope(cx, &value.toObject());
...
void SendRunnable::RunOnMainThread(ErrorResult& aRv) {
  ...
  if (!mBlobImpl) {
    ...
    MC::Rooted<JSObject*> globalObject(RootingCx(),
