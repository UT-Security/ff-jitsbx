...
already_AddRefed<JSActor> JSActorManager::GetActor(JSContext* aCx,
  ...
  JS::sandbox::Rooted<JSObject*> actorObj(aCx);
  if (side.mModuleURI || side.mESModuleURI) {
    JS::sandbox::Rooted<JSObject*> exports(aCx);
    if (side.mModuleURI) {
      JS::sandbox::Rooted<JSObject*> global(aCx);
    ...
    JS::sandbox::Rooted<JS::Value> ctor(aCx);
...
void JSActorManager::ReceiveRawMessage(
  ...
  {
    JS::sandbox::Rooted<JS::Value> stackVal(cx);
    if (aStack) {
  ...
  JS::sandbox::Rooted<JS::Value> data(cx);
  if (aData) {
