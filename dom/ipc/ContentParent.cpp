...
bool ContentParent::InitInternal(ProcessPriority aInitialPriority) {
  ...
  if (ssm) {
    ...
    if (ParentProcessMessageManager* mm =
      ...
      JS::sandbox::Rooted<JS::Value> init(jsapi.cx());
...
mozilla::ipc::IPCResult ContentParent::RecvScriptErrorInternal(
  ...
  if (aStack) {
    ...
    JS::sandbox::Rooted<JSObject*> stackObj(cx, &stack.toObject());
    MOZ_ASSERT(JS::IsUnwrappedSavedFrame(stackObj));

    JS::sandbox::Rooted<JSObject*> stackGlobal(cx, JS::GetNonCCWObjectGlobal(stackObj));
    msg = new nsScriptErrorWithStack(JS::GetNothingHandleValue(), stackObj,
                                     stackGlobal);
  } else {
