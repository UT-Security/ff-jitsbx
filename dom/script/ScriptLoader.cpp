...
nsresult ScriptLoader::EvaluateScript(nsIGlobalObject* aGlobalObject,
  ...
  if (rv == NS_OK) {
    MC::Rooted<JSScript*> script(cx, exec.GetScript());
...
void ScriptLoader::EncodeRequestBytecode(MCContext* aCx,
  ...
  if (aRequest->IsModuleRequest()) {
    ...
    MC::Rooted<JSScript*> script(aCx, aRequest->mScriptForBytecodeEncoding);
    result =
  ...
  while (!mBytecodeEncodingQueue.isEmpty()) {
    ...
    if (aes.isSome()) {
      if (request->IsModuleRequest()) {
        ...
        MC::Rooted<JSObject*> module(aes->cx(), moduleScript->ModuleRecord());
        JS::AbortIncrementalEncoding(module);
      } else {
        MC::Rooted<JSScript*> script(aes->cx(),
