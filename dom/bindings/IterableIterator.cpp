...
already_AddRefed<Promise> AsyncIterableNextImpl::NextSteps(
  ...
  if (aObject->mIsFinished) {
    ...
    MC::Rooted<JS::Value> dict(aCx);
    iterator_utils::DictReturn(aCx, &dict, true, MC::UndefinedHandleValue(), aRv);
    if (aRv.Failed()) {
  ...
  auto fulfillSteps = [](MCContext* aCx, JS::Handle<JS::Value> aNext,
    ...
    if (aNext.isMagic(binding_details::END_OF_ITERATION)) {
      ...
      iterator_utils::DictReturn(aCx, &dict, true, MC::UndefinedHandleValue(),
                                 aRv);
      if (aRv.Failed()) {
        return nullptr;
      }
