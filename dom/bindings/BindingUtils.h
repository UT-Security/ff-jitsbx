...
class ProtoAndIfaceCache {
  ...
  class ArrayCache
      : public Array<JS::sandbox::Heap<JSObject*>, kProtoAndIfaceCacheCount> {
   public:
    ...
    JS::sandbox::Heap<JSObject*>& EntrySlotOrCreate(size_t i) { return (*this)[i]; }

    JS::sandbox::Heap<JSObject*>& EntrySlotMustExist(size_t i) { return (*this)[i]; }
  ...
  class PageTableCache {
   public:
    ...
    JS::sandbox::Heap<JSObject*>& EntrySlotOrCreate(size_t i) {
      MOZ_ASSERT(i < kProtoAndIfaceCacheCount);
      size_t pageIndex = i / kPageSize;
      size_t leafIndex = i % kPageSize;
      Page* p = mPages[pageIndex];
    ...
    JS::sandbox::Heap<JSObject*>& EntrySlotMustExist(size_t i) {
      MOZ_ASSERT(i < kProtoAndIfaceCacheCount);
      size_t pageIndex = i / kPageSize;
      size_t leafIndex = i % kPageSize;
      Page* p = mPages[pageIndex];
   ...
   private:
    ...
    typedef Array<JS::sandbox::Heap<JSObject*>, kPageSize> Page;
...
inline bool WrapNewBindingNonWrapperCachedObject(
  ...
  JS::sandbox::Rooted<JSObject*> obj(cx);
  {
    ...
    JS::sandbox::Rooted<JSObject*> scope(cx, scopeArg);
    JS::sandbox::Rooted<JSObject*> proto(cx, givenProto);
...
inline bool FindEnumStringIndex(BindingCallContext& cx, JS::Handle<JS::Value> v,
  ...
  JS::sandbox::Rooted<JSString*> str(cx, JS::ToString(cx, v));
  if (!str) {
    return false;
  }
...
inline bool ConvertIdToString(JSContext* cx, JS::Handle<JS::PropertyKey> id,
  ...
  if (MOZ_LIKELY(id.isString())) {
    ...
    JS::sandbox::Rooted<JS::Value> nameVal(cx, js::IdToValue(id));
...
already_AddRefed<T> ConstructJSImplementation(const char* aContractId,
                                              nsIGlobalObject* aGlobal,
                                              ErrorResult& aRv) {
  JS::sandbox::RootingContext* cx = RootingCx();
  JS::sandbox::Rooted<JSObject*> jsImplObj(cx);
  ...
  JS::sandbox::Rooted<JSObject*> jsImplGlobal(cx, JS::GetNonCCWObjectGlobal(jsImplObj));
...
class MOZ_STACK_CLASS BindingJSObjectCreator {
 ...
 private:
  ...
  JS::sandbox::Rooted<JSObject*> mReflector;
