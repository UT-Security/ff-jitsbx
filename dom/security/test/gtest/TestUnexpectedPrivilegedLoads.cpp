...
TEST_F(TelemetryTestFixture, UnexpectedPrivilegedLoadsTelemetryTest) {
  ...
  for (auto const& currentTest : myTestCases) {
    ...
    MC::Rooted<JS::Value> eventsSnapshot(cx.GetJSContext());
    ...
    MC::Rooted<JS::Value> extraValP(aCx);
    ASSERT_TRUE(
    ...
    MC::Rooted<JS::Value> extraValF(aCx);
    ASSERT_TRUE(
    ...
    MC::Rooted<JS::Value> extraValRedirects(aCx);
