...
nsresult GetJSValFromKeyPathString(
  ...
  *aKeyJSVal = aValue;
  ...
  MC::Rooted<JSObject*> targetObject(aCx, nullptr);
  MC::Rooted<JS::Value> currentVal(aCx, aValue);
  ...
  while (tokenizer.hasMoreTokens()) {
    ...
    if (!targetObject) {
      ...
      MC::Rooted<mozilla::Maybe<JS::PropertyDescriptor>> desc(aCx);
      QM_TRY(OkIf(JS_GetOwnUCPropertyDescriptor(aCx, obj, keyPathChars,
                                                keyPathLen, &desc)),
             NS_ERROR_DOM_INDEXEDDB_UNKNOWN_ERR,
             IDB_REPORT_INTERNAL_ERR_LAMBDA);

      MC::Rooted<JS::Value> intermediate(aCx);
      bool hasProp = false;
    ...
    if (targetObject) {
      ...
      if (tokenizer.hasMoreTokens()) {
        ...
        // chain.
        MC::Rooted<JSObject*> dummy(aCx, JS_NewPlainObject(aCx));
        if (!dummy) {
...
nsresult KeyPath::ExtractKeyAsJSVal(MCContext* aCx, const JS::Value& aValue,
  ...
  MC::Rooted<JSObject*> arrayObj(aCx, JS::NewArrayObject(aCx, len));
  if (!arrayObj) {
...
nsresult KeyPath::ToJSVal(MCContext* aCx,
  ...
  if (IsArray()) {
    uint32_t len = mStrings.Length();
    MC::Rooted<JSObject*> array(aCx, JS::NewArrayObject(aCx, len));
    if (!array) {
