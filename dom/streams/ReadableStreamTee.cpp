...
void ReadableStreamDefaultTeeReadRequest::ChunkSteps(
  ...
  class ReadableStreamDefaultTeeReadRequestChunkSteps
    ...
    JS::sandbox::PersistentRooted<JS::Value> mChunk;
   ...
   public:
    ...
    void Run(AutoSlowOperation& aAso) override {
      ...
      // Step 2.
      JS::sandbox::Rooted<JS::Value> chunk1(cx, mChunk);
      JS::sandbox::Rooted<JS::Value> chunk2(cx, mChunk);
...
MOZ_CAN_RUN_SCRIPT void ByteStreamTeePullAlgorithm(JSContext* aCx,
  ...
  if (!byobRequest) {
    ...
    JS::sandbox::Rooted<JSObject*> view(aCx, byobRequest->View());
 ...
 public:
  ...
  void ChunkSteps(JSContext* aCx, JS::Handle<JS::Value> aChunk,
    ...
    class PullWithDefaultReaderChunkStepMicrotask : public MicroTaskRunnable {
      RefPtr<TeeState> mTeeState;
      JS::sandbox::PersistentRooted<JSObject*> mChunk;
     ...
     public:
      ...
      void Run(AutoSlowOperation& aAso) override {
        ...
        JS::sandbox::Rooted<JSObject*> chunk1(cx, mChunk);
        JS::sandbox::Rooted<JSObject*> chunk2(cx, mChunk);

        // Step 4. If canceled1 is false and canceled2 is false,
        ErrorResult rv;
        if (!mTeeState->Canceled1() && !mTeeState->Canceled2()) {
          // Step 4.1. Let cloneResult be CloneAsUint8Array(chunk).
          JS::sandbox::Rooted<JSObject*> cloneResult(cx, CloneAsUint8Array(cx, mChunk));
...
class PullWithBYOBReader_ReadIntoRequest final : public ReadIntoRequest {
 ...
 public:
  ...
  void ChunkSteps(JSContext* aCx, JS::Handle<JS::Value> aChunk,
     ...
     public:
      ...
      void Run(AutoSlowOperation& aAso) override {
        ...
        if (!otherCanceled) {
          ...
          JS::sandbox::Rooted<JSObject*> clonedChunk(cx, CloneAsUint8Array(cx, mChunk));

          // Step 5.2. If cloneResult is an abrupt completion,
          if (!clonedChunk) {
            JS::sandbox::Rooted<JS::Value> exception(cx);
    ...
    JS::sandbox::Rooted<JSObject*> chunk(aCx, aChunk.toObjectOrNull());
  ...
  void CloseSteps(JSContext* aCx, JS::Handle<JS::Value> aChunk,
    ...
    if (!aChunk.isUndefined()) {
      ...
      JS::sandbox::Rooted<JSObject*> chunkObject(aCx, &aChunk.toObject());
