...
static bool ByteLengthQueuingStrategySize(JSContext* cx, unsigned argc,
  ...
  JS::sandbox::Rooted<JSObject*> chunkObj(cx, JS::ToObject(cx, args.get(0)));
  if (!chunkObj) {
    return false;
  }
...
already_AddRefed<Function> ByteLengthQueuingStrategy::GetSize(
  ...
  JS::sandbox::Rooted<JSFunction*> sizeFunction(
  ...
  JS::sandbox::Rooted<JSObject*> funObj(cx, JS_GetFunctionObject(sizeFunction));
  JS::sandbox::Rooted<JSObject*> global(cx, mGlobal->GetGlobalJSObject());
