...
void AbortStream(MCContext* aCx, ReadableStream* aReadableStream,
  ...
  MC::Rooted<JS::Value> value(aCx, aReasonDetails);
...
already_AddRefed<Promise> FetchRequest(nsIGlobalObject* aGlobal,
  ...
  if (signalImpl && signalImpl->Aborted()) {
    ...
    MC::Rooted<JS::Value> reason(cx, signalImpl->RawReason());
...
already_AddRefed<Promise> FetchBody<Derived>::ConsumeBody(
  ...
  if (signalImpl && signalImpl->Aborted()) {
    MC::Rooted<JS::Value> abortReason(aCx, signalImpl->RawReason());
...
void FetchBody<Derived>::RunAbortAlgorithm() {
  ...
  MC::Rooted<JS::Value> abortReason(cx);
