...
void ConsoleUtils::ReportForServiceWorkerScopeInternal(
  ...
  JS::sandbox::Rooted<JSObject*> global(cx, GetOrCreateSandbox(cx));
  ...
  JS::sandbox::Rooted<JS::Value> messageValue(cx);
  ...
  JS::sandbox::Rooted<JS::Value> eventValue(cx);
  if (!ToJSValue(cx, event, &eventValue)) {
    return;
  }

  // This is a legacy property.
  JS::sandbox::Rooted<JSObject*> eventObj(cx, &eventValue.toObject());
