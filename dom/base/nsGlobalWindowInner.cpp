...
class PromiseDocumentFlushedResolver final {
 public:
  ...
  void Call() {
    ...
    ErrorResult error;
    MC::Rooted<JS::Value> returnVal(RootingCx());
...
nsresult nsGlobalWindowInner::DefineArgumentsProperty(nsIArray* aArguments) {
  ...
  MC::Rooted<JSObject*> obj(RootingCx(), GetWrapperPreserveColor());
...
bool nsGlobalWindowInner::ResolveComponentsShim(
  ...
  MC::Rooted<JSObject*> components(aCx, JS_NewPlainObject(aCx));
  if (NS_WARN_IF(!components)) {
    return false;
  }

  // Create a fake interfaces object.
  MC::Rooted<JSObject*> interfaces(aCx, JS_NewPlainObject(aCx));
  ...
  for (uint32_t i = 0; i < ArrayLength(kInterfaceShimMap); ++i) {
    ...
    MC::Rooted<JS::Value> v(aCx, JS::UndefinedValue());
...
bool nsGlobalWindowInner::DoResolve(
  ...
  if ((aId == XPCJSRuntime::Get()->GetStringID(XPCJSContext::IDX_CONTROLLERS) ||
    ...
    MC::Rooted<JSObject*> shim(aCx, JS_NewObject(aCx, clazz));
    if (NS_WARN_IF(!shim)) {
      return false;
    }
...
void nsGlobalWindowInner::GetOwnPropertyNames(
  ...
  MC::Rooted<JSObject*> wrapper(aCx, GetWrapper());
...
nsresult nsGlobalWindowInner::DispatchSyncPopState() {
  ...
  MC::Rooted<JS::Value> stateJSValue(cx);
...
void nsGlobalWindowInner::RedefineProperty(MCContext* aCx,
                                           ...
                                           JS::Handle<JS::Value> aValue,
                                           ErrorResult& aError) {
  MC::Rooted<JSObject*> thisObj(aCx, GetWrapperPreserveColor());
  if (!thisObj) {
    aError.Throw(NS_ERROR_UNEXPECTED);
    return;
  }
...
void nsGlobalWindowInner::FireOnNewGlobalObject() {
  ...
  MC::Rooted<JSObject*> global(aes.cx(), GetWrapper());
