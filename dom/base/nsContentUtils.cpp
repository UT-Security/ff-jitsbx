...
static void ReportPatternCompileFailure(nsAString& aPattern,
  ...
  JS::sandbox::Rooted<JSString*> messageStr(cx, messageVal.toString());
...
Maybe<bool> nsContentUtils::IsPatternMatching(nsAString& aValue,
  ...
  JS::sandbox::Rooted<JS::Value> rval(cx, JS::NullValue());
  if (!aHasMultiple) {
    size_t idx = 0;
...
static void DoCustomElementCreate(Element** aElement, JSContext* aCx,
                                  ...
                                  ErrorResult& aRv) {
  JS::sandbox::Rooted<JS::Value> constructResult(aCx);
...
nsresult nsContentUtils::CreateJSValueFromSequenceOfObject(
  ...
  JS::sandbox::Rooted<JSObject*> array(aCx, JS::NewArrayObject(aCx, aTransfer.Length()));
  if (!array) {
    return NS_ERROR_OUT_OF_MEMORY;
  }

  for (uint32_t i = 0; i < aTransfer.Length(); ++i) {
    JS::sandbox::Rooted<JSObject*> object(aCx, aTransfer[i]);
    if (!object) {
      continue;
    }
...
static nsGlobalWindowInner* GetInnerWindowForGlobal(nsIGlobalObject* aGlobal) {
  ...
  JS::sandbox::Rooted<JSObject*> scope(RootingCx(), aGlobal->GetGlobalJSObject());
