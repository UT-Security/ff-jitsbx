...
int32_t CustomElementRegistry::InferNamespace(
  ...
  MC::Rooted<JSObject*> proto(aCx, constructor);
  while (proto) {
...
void CustomElementRegistry::Define(
  ...
  {  // Set mIsCustomDefinitionRunning.
    ...
    // object.
    MC::Rooted<JS::Value> prototype(aCx);
    ...
    MC::Rooted<JS::Value> formAssociatedValue(aCx);
  ...
  // Dispatch a "customelementdefined" event for DevTools.
  {
    ...
    MC::Rooted<JS::Value> detail(aCx, JS::StringValue(nameJsStr));
...
static void DoUpgrade(Element* aElement, CustomElementDefinition* aDefinition,
  ...
  MC::Rooted<JS::Value> constructResult(RootingCx());
...
already_AddRefed<nsISupports> CustomElementRegistry::CallGetCustomInterface(
  ...
  AutoJSAPI jsapi;
  MC::Rooted<JSObject*> funcGlobal(RootingCx(), func->CallbackGlobalOrNull());
  ...
  MC::Rooted<JSObject*> customInterface(cx);
