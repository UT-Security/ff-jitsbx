...
{
  JS::sandbox::Rooted<JSObject*> globalObject(
...
TEST(DOM_Base_ContentUtils, StringifyJSON_Object_UndefinedIsNullStringLiteral)
{
  ...
  JS::sandbox::Rooted<JSObject*> jsObj(cx, JS_NewPlainObject(cx));
  JS::sandbox::Rooted<JSString*> valueStr(cx, JS_NewStringCopyZ(cx, "Hello World!"));
  ASSERT_TRUE(JS_DefineProperty(cx, jsObj, "key1", valueStr, JSPROP_ENUMERATE));
  JS::sandbox::Rooted<JS::Value> jsValue(cx, JS::ObjectValue(*jsObj));
