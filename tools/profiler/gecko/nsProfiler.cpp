...
nsProfiler::GetSharedLibraries(JSContext* aCx,
  ...
  JS::sandbox::Rooted<JSObject*> obj(aCx, &val.toObject());
  if (!obj) {
    return NS_ERROR_FAILURE;
  }
...
nsProfiler::GetActiveConfiguration(JSContext* aCx,
  ...
  if (jsValue.isNull()) {
    ...
    JS::sandbox::Rooted<JSObject*> obj(aCx, &jsValue.toObject());
    if (!obj) {
      return NS_ERROR_FAILURE;
    }
    aResult.setObject(*obj);
  ...
  StartGathering(aSinceTime)
      ->Then(
          ...
          [promise](const mozilla::ProfileAndAdditionalInformation& aResult) {
            ...
            if (typedArray) {
              JS::sandbox::Rooted<JS::Value> val(cx, JS::ObjectValue(*typedArray));
              promise->MaybeResolve(val);
            } else {
  ...
  StartGathering(aSinceTime)
      ->Then(
          ...
          [promise](const mozilla::ProfileAndAdditionalInformation& aResult) {
            ...
            JS::sandbox::Rooted<JS::Value> typedArrayValue(cx,
                                                  JS::ObjectValue(*typedArray));
            // Get the additional information object.
            JS::sandbox::Rooted<JS::Value> additionalInfoVal(cx);
            ...
            JS::sandbox::Rooted<JSObject*> resultObj(cx, JS_NewPlainObject(cx));
