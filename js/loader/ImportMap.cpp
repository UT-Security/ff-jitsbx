...
static UniquePtr<SpecifierMap> SortAndNormalizeSpecifierMap(
  ...
  JS::sandbox::Rooted<JS::IdVector> specifierKeys(aCx, JS::IdVector(aCx));
  ...
  for (size_t i = 0; i < specifierKeys.length(); i++) {
    const JS::sandbox::RootedId specifierId(aCx, specifierKeys[i]);
    ...
    JS::sandbox::RootedValue idVal(aCx);
...
static UniquePtr<ScopeMap> SortAndNormalizeScopes(
    JSContext* aCx, JS::HandleObject aOriginalMap, nsIURI* aBaseURL,
    const ReportWarningHelper& aWarning) {
  JS::sandbox::Rooted<JS::IdVector> scopeKeys(aCx, JS::IdVector(aCx));
  ...
  for (size_t i = 0; i < scopeKeys.length(); i++) {
    const JS::sandbox::RootedId scopeKey(aCx, scopeKeys[i]);
    ...
    JS::sandbox::RootedValue mapVal(aCx);
    ...
    JS::sandbox::RootedObject potentialSpecifierMap(aCx, &mapVal.toObject());
...
UniquePtr<ImportMap> ImportMap::ParseString(
  ...
  // input.
  JS::sandbox::Rooted<JS::Value> parsedVal(aCx);
  if (!JS_ParseJSON(aCx, aInput.get(), aInput.length(), &parsedVal)) {
    ...
    JS::sandbox::Rooted<JSObject*> obj(aCx, &exn.toObject());
  ...
  JS::sandbox::RootedObject parsedObj(aCx, &parsedVal.toObject());
  JS::sandbox::RootedValue importsVal(aCx);
  ...
  if (!importsVal.isUndefined()) {
    ...
    JS::sandbox::RootedObject importsObj(aCx, &importsVal.toObject());
  ...
  JS::sandbox::RootedValue scopesVal(aCx);
  ...
  if (!scopesVal.isUndefined()) {
    ...
    JS::sandbox::RootedObject scopesObj(aCx, &scopesVal.toObject());
  ...
  for (size_t i = 0; i < keys.length(); i++) {
    const JS::sandbox::RootedId key(aCx, keys[i]);
    nsAutoJSString val;
