...
JSObject* ModuleLoaderBase::HostResolveImportedModule(
  ...
  JS::sandbox::Rooted<JSObject*> module(aCx);

  {
    ...
    JS::sandbox::Rooted<JSString*> specifierString(
...
bool ModuleLoaderBase::ImportMetaResolve(JSContext* cx, unsigned argc,
  ...
  JS::sandbox::RootedValue modulePrivate(
  ...
  JS::sandbox::RootedValue v(cx, args.get(ImportMetaResolveSpecifierArg));
  JS::sandbox::RootedString specifier(cx, JS::ToString(cx, v));
  if (!specifier) {
    return false;
  }

  // Step 4.2, 4.3 are implemented in ImportMetaResolveImpl.
  JS::sandbox::RootedString url(cx, ImportMetaResolveImpl(cx, modulePrivate, specifier));
  if (!url) {
    return false;
  }
...
JSString* ModuleLoaderBase::ImportMetaResolveImpl(
  ...
  JS::sandbox::RootedString urlString(aCx);
...
bool ModuleLoaderBase::HostPopulateImportMeta(
  ...
  JS::sandbox::Rooted<JSString*> urlString(aCx, JS_NewStringCopyZ(aCx, url.get()));
  if (!urlString) {
    JS_ReportOutOfMemory(aCx);
    return false;
  }
  ...
  JS::sandbox::RootedObject resolveFuncObj(aCx, JS_GetFunctionObject(resolveFunc));
...
ModuleLoaderBase* ModuleLoaderBase::GetCurrentModuleLoader(JSContext* aCx) {
  ...
  JS::sandbox::Rooted<JSObject*> object(aCx, JS::CurrentGlobalOrNull(aCx));
  if (!object) {
    return nullptr;
  }
...
nsresult ModuleLoaderBase::HandleResolveFailure(
  ...
  JS::sandbox::Rooted<JSString*> string(aCx, JS_NewUCStringCopyZ(aCx, errorText.get()));
  if (!string) {
    return NS_ERROR_OUT_OF_MEMORY;
  }

  if (!JS::CreateError(aCx, JSEXN_TYPEERR, nullptr, filename, aLineNumber,
                       aColumnNumber, nullptr, string, JS::GetNothingHandleValue(),
                       aErrorOut)) {
    return NS_ERROR_OUT_OF_MEMORY;
  }
...
nsresult ModuleLoaderBase::ResolveRequestedModules(
  ...
  JSContext* cx = jsapi.cx();
  JS::sandbox::Rooted<JSObject*> moduleRecord(cx, ms->ModuleRecord());
...
void ModuleLoaderBase::FinishDynamicImport(
  ...
  JS::sandbox::Rooted<JS::Value> referencingScript(aCx,
                                          aRequest->mDynamicReferencingPrivate);
  JS::sandbox::Rooted<JSString*> specifier(aCx, aRequest->mDynamicSpecifier);
  JS::sandbox::Rooted<JSObject*> promise(aCx, aRequest->mDynamicPromise);

  JS::sandbox::Rooted<JSObject*> moduleRequest(aCx,
...
bool ModuleLoaderBase::InstantiateModuleGraph(ModuleLoadRequest* aRequest) {
  ...
  JS::sandbox::Rooted<JSObject*> module(jsapi.cx(), moduleScript->ModuleRecord());
  ...
  if (!JS::ModuleLink(jsapi.cx(), module)) {
    ...
    JS::sandbox::RootedValue exception(jsapi.cx());
...
nsresult ModuleLoaderBase::InitDebuggerDataForModuleGraph(
  ...
  JS::sandbox::Rooted<JSScript*> script(aCx, JS::GetModuleScript(module));
...
nsresult ModuleLoaderBase::EvaluateModuleInContext(
  ...
  if (moduleScript->HasErrorToRethrow()) {
    ...
    JS::sandbox::Rooted<JS::Value> error(aCx, moduleScript->ErrorToRethrow());
    JS_SetPendingException(aCx, error);
  ...
  JS::sandbox::Rooted<JSObject*> evaluationPromise(aCx);
  if (rval.isObject()) {
