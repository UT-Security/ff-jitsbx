...
size_t js::gc::TenuringTracer::moveSlotsToTenured(NativeObject* dst,
...

  {
    ...
    // ask2374
    HeapSlot* allocation =
        zone->pod_sandbox_malloc<HeapSlot>(ObjectSlots::allocCount(count));
    // ask2374
    if (!allocation) {
...
size_t js::gc::TenuringTracer::moveElementsToTenured(NativeObject* dst,
  ...
  ObjectElements* dstHeader;
  {
    ...
    // ask2374
    dstHeader =
        reinterpret_cast<ObjectElements*>(zone->pod_sandbox_malloc<HeapSlot>(nslots));
    // ask2374
    if (!dstHeader) {
  ...
  if (src->hasHeapDigits()) {
    ...
    if (!nursery().isInside(src->heapDigits_)) {
      ...
      Zone* zone = src->nurseryZone();
      {
        AutoEnterOOMUnsafeRegion oomUnsafe;
        // ask2374
        dst->heapDigits_ = zone->pod_sandbox_malloc<JS::BigInt::Digit>(length);
        // ask2374
        if (!dst->heapDigits_) {
