...
void AssemblerX86Shared::processCodeLabelsInPlace(uint8_t* rawCode) {
  ...
  for (const CodeLabel& label : codeLabels_) {
    BindInPlace(rawInPlace, rawCode, label);
  }
}

void AssemblerX86Shared::processDataLabelsInPlace(uint8_t* rawCode, JitCode* code) {
  // Set the absolute address of each GC::Cell* in the code.
  uint8_t* rawInPlace = (uint8_t*)masm.buffer();
  uint8_t* dataPtr = code->dataSection();
  size_t index = 0;
  ...
  for (size_t i = 0; i < dataValueSection_.length(); i++) {
    intptr_t offset = dataValueSection_[i].first.offset();
    X86Encoding::SetPointer(rawInPlace + offset, &dataPtr[index]);
    index += sizeof(Value);
  }
  dataValueSection_.clear();
...
AutoBundleInstructionScope::AutoBundleInstructionScope(AssemblerX86Shared& masm): masm(masm), active_(true) {
  nested_ = !masm.beginBundleInstruction();
}

void AutoBundleInstructionScope::end() {
  MOZ_ASSERT(active_, "Unexpected inactive instruction bundle");
  if (!nested_) masm.endBundleInstruction();
  active_ = false;
}
...
AutoBundleGroupScope::AutoBundleGroupScope(AssemblerX86Shared& masm): masm(masm), active_(true) {
  nested_ = !masm.beginBundleGroup();
}
...
#else
AutoBundleInstructionScope::AutoBundleInstructionScope(AssemblerX86Shared& masm) {}
void AutoBundleInstructionScope::end() {}
AutoBundleInstructionScope::~AutoBundleInstructionScope() {}
