...
void Assembler::ClearLoad64Address(CodeLocationLabel label) {
  Instruction* i = (Instruction*)label.raw();
  ClearLoad64Address(i);
}

void Assembler::ClearLoad64Address(Instruction* inst0) {
  MOZ_ASSERT(inst0->IsADRP());
  Instruction* inst1 = inst0->NextInstruction();
  MOZ_ASSERT(inst1->IsLoad());
  MOZ_ASSERT(inst0->Rd() == inst1->Rd());
...
void Assembler::UpdateLoad64Address(Instruction* inst0, Instruction* target, uint64_t* address) {
  ...
  adrp(inst0, dest, valueOffset - loadOffset);
  
  ptrdiff_t pageOffset =
      reinterpret_cast<uintptr_t>(address) -
      ((reinterpret_cast<uintptr_t>(address) >> vixl::kPageSizeLog2)
       << vixl::kPageSizeLog2);

  ldr(inst1, dest, MemOperand(dest, pageOffset));
}

void Assembler::ClearLoadF64Address(CodeLocationLabel label) {
  Instruction* i = (Instruction*)label.raw();
  ClearLoadF64Address(i);
}

void Assembler::ClearLoadF64Address(Instruction* inst0) {
  MOZ_ASSERT(inst0->IsADRP());
  Instruction* inst1 = inst0->NextInstruction();
  MOZ_ASSERT(inst1->IsLoad());
  ...
  vixl::VRegister dest = vixl::VRegister(inst1->Rd(), 64);
...
void Assembler::UpdateLoadF64Address(Instruction* inst0, Instruction* target, double* address) {
  ...
  vixl::Register base = vixl::Register(inst0->Rd(), 64);
  adrp(inst0, base, valueOffset - loadOffset);
  ...
  vixl::VRegister dest = vixl::VRegister(inst1->Rd(), 64);
  ldr(inst1, dest, MemOperand(base, pageOffset));
}

void Assembler::ClearLoadF32Address(CodeLocationLabel label) {
  Instruction* i = (Instruction*)label.raw();
  ClearLoadF64Address(i);
}

void Assembler::ClearLoadF32Address(Instruction* inst0) {
  MOZ_ASSERT(inst0->IsADRP());
  Instruction* inst1 = inst0->NextInstruction();
  MOZ_ASSERT(inst1->IsLoad());
  ...
  vixl::VRegister dest = vixl::VRegister(inst1->Rd(), 32);
  ldr(inst1, dest, MemOperand(base, 0));
}
...
/* static */
void Assembler::TraceGCDataSection(JSTracer* trc, JitCode* code) {
  uintptr_t* words = reinterpret_cast<uintptr_t*>(code->gcDataSection());
#ifdef JS_PUNBOX64
  Value* values = reinterpret_cast<Value*>(words);
#endif
  gc::Cell** cells = reinterpret_cast<gc::Cell**>(words);

  for (size_t i = 0; i < code->gcDataEntries(); i++) {
#ifdef JS_PUNBOX64
    ...
    // is not zero-tagged, we have to interpret it as a Value to ensure that th
    ...
    if (words[i] >> JSVAL_TAG_SHIFT) {
      ...
      TraceManuallyBarrieredEdge(trc, &values[i], "masm-data-value");
      continue;
    }
#endif
