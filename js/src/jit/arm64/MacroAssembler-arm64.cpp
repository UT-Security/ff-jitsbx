...
void MacroAssembler::patchNopToCall(uint8_t* call, uint8_t* target) {
  ...
  sys_jitcode_modify(inst,
                     vixl::BL | ImmUncondBranch((target - inst) >> 2),
                     sizeof(int32_t));
#else
...
void MacroAssembler::patchCallToNop(uint8_t* call) {
  ...
  MOZ_ASSERT(instr->IsBL() || instr->IsNOP());
#ifdef JS_SANDBOX_LFI_JIT_MEMORY
  sys_jitcode_modify(inst, vixl::HINT | ImmHint(vixl::NOP),
                     sizeof(int32_t));
#else
  nop(instr);
#endif
...
void MacroAssembler::enterFakeExitFrameForWasm(Register cxreg, Register scratch,
  ...
  Add(SandboxTemporaryReg64, sp, 8);
  And(SandboxOffsetReg64, SandboxTemporaryReg64, SANDBOX_MASK);
  Add(sp, SandboxBaseReg64, SandboxOffsetReg64);
#else
  Add(sp, sp, 8);
#endif
