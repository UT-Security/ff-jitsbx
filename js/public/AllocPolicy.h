...
class AllocPolicyBase {
 public:
  ...
  T* maybe_pod_malloc(size_t numElems) {
    ...
    return maybe_pod_arena_malloc<T>(mc::GetMallocArena(), numElems);
#else
...
#ifdef JS_SANDBOX_API
    return maybe_pod_arena_calloc<T>(mc::GetMallocArena(), numElems);
#else
...
#ifdef JS_SANDBOX_API
    return maybe_pod_arena_realloc<T>(mc::GetMallocArena(), p, oldSize, newSize);
#else
...
#ifdef JS_SANDBOX_API
    return pod_arena_malloc<T>(mc::GetMallocArena(), numElems);
#else
...
#ifdef JS_SANDBOX_API
    return pod_arena_calloc<T>(mc::GetMallocArena(), numElems);
#else
...
#ifdef JS_SANDBOX_API
    return pod_arena_realloc<T>(mc::GetMallocArena(), p, oldSize, newSize);
#else
...
#ifdef JS_SANDBOX_API
    return pod_arena_realloc<T>(mc::GetMallocArena(), prior, oldSize, newSize);
#else
