...
bool name::finalizeInBackground(                                                 \
...
bool name::construct(JSContext* cx,                                              \
                                              JS::HandleObject proxy,            \
                                              const JS::CallArgs& args) const {  \
...
bool name::getElements(JSContext* cx,                                            \
                                                JS::HandleObject proxy,          \
                                                uint32_t begin, uint32_t end,    \
                                                ElementAdder* adder) const {     \
  return ops->getElements(handler, cx, proxy, begin, end, adder);                \
}                                                                                \
bool name::isScripted() const {                                                  \
  return ops->isScripted(handler);                                               \
}

#define DECLARE_PROXY_HANDLER_OPS_CLASS(name, base)                          \
class JS_PUBLIC_API name : public base {                                     \
  const ProxyHandlerOps* pOps_;                                              \
  const void* private_;                                                      \
public:                                                                      \
  explicit name(const ProxyHandlerOps* pOps, const void* priv,               \
                            const void* aFamily, bool aHasPrototype = false, \
                            bool aHasSecurityPolicy = false);                \
  const ProxyHandlerOps* getOps() const { return pOps_; }                    \
  const void* getPrivate() const { return private_; }                        \
  DECLARE_PROXY_HANDLER_OPS_METHODS                                          \
};

#define DEFINE_PROXY_HANDLER_OPS_CLASS(name, base)                            \
name::name(const ProxyHandlerOps* pOps,                                       \
                                     const void* priv,                        \
                                     const void* aFamily,                     \
                                     bool aHasPrototype,                      \
                                     bool aHasSecurityPolicy)                 \
     : base(aFamily, aHasPrototype, aHasSecurityPolicy),                      \
       pOps_(pOps), private_(priv) {}                                         \
DEFINE_PROXY_HANDLER_OPS_METHODS(name, pOps_, private_)

DECLARE_PROXY_HANDLER_OPS_CLASS(BaseProxyHandlerWithOps, BaseProxyHandler)
#endif
