...
JSObject* nsXPCWrappedJS::CallQueryInterfaceOnJSObject(MCContext* cx,
  ...
  MC::RootedObject jsobj(cx, jsobjArg);
  MC::RootedValue arg(cx);
  MC::RootedValue retval(cx);
  MC::RootedObject retObj(cx);
  MC::RootedValue fun(cx);
  ...
  if (!success && JS_IsExceptionPending(cx)) {
    MC::RootedValue jsexception(cx, NullValue());

    if (JS_GetPendingException(cx, &jsexception)) {
      if (jsexception.isObject()) {
        // XPConnect may have constructed an object to represent a
        // C++ QI failure. See if that is the case.
        MC::Rooted<JSObject*> exceptionObj(cx, &jsexception.toObject());
        Exception* e = nullptr;
...
nsresult nsXPCWrappedJS::DelegatedQueryInterface(REFNSIID aIID,
  ...
  MC::RootedObject obj(RootingCx(), GetJSObject());
  ...
  if (aIID.Equals(NS_GET_IID(nsISupportsWeakReference))) {
    ...
    MC::RootedObject rootScope(ccx, root->GetJSObjectGlobal());
...
// static
JSObject* nsXPCWrappedJS::GetRootJSObject(MCContext* cx, JSObject* aJSObjArg) {
...
nsresult nsXPCWrappedJS::CheckForException(XPCCallContext& ccx,
  ...
  MC::RootedValue js_exception(cx);
...
nsXPCWrappedJS::CallMethod(uint16_t methodIndex, const nsXPTMethodInfo* info,
  ...
  MC::RootedValue fval(cx);
  MC::RootedObject thisObj(cx, obj);

  MC::RootedValueVector args(cx);
  AutoScriptEvaluate scriptEval(cx);
  ...
  for (i = 0; i < argc; i++) {
    ...
    MC::RootedValue val(cx, NullValue());
...
pre_call_clean_up:
  ...
  for (i = 0; i < paramCount; i++) {
    ...
    if (&param == info->GetRetval()) {
      ...
      MC::RootedObject obj(cx, &argv[i].toObject());
...
bool xpc::IsOutObject(MCContext* cx, JSObject* obj) {
  return JS::GetClass(obj) == &XPCOutParamClass;
}

JSObject* xpc::NewOutObject(MCContext* cx) {
