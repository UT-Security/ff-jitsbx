...
nsresult XPCWrappedNative::WrapNewGlobal(JSContext* cx,
  ...
  // Create the global.
  aOptions.creationOptions().setTrace((JSTraceOp)sbx_register_cb((void*)XPCWrappedNative::Trace, 0));
  ...
  JS::sandbox::RootedObject protoObj(cx, proto->GetJSProtoObject());
...
nsresult XPCWrappedNative::GetNewOrUsed(JSContext* cx, xpcObjectHelper& helper,
  ...
  if (scrWrapper && scrWrapper->WantPreCreate()) {
    ...
    if (cache) {
      JS::sandbox::RootedObject cached(cx, cache->GetWrapper());
      if (cached) {
...
bool XPCWrappedNative::Init(JSContext* cx, nsIXPCScriptable* aScriptable) {
  ...
  JS::sandbox::RootedObject protoJSObject(cx, HasProto() ? GetProto()->GetJSProtoObject()
...
nsresult XPCWrappedNative::InitTearOff(JSContext* cx,
  ...
  if (wrappedJS) {
    JS::sandbox::RootedObject jso(cx, wrappedJS->GetJSObject());
...
bool XPCWrappedNative::CallMethod(XPCCallContext& ccx,
  ...
  JS::sandbox::Rooted<CallMethodHelper> helper(ccx, /* init = */ ccx);
...
CallMethodHelper::~CallMethodHelper() {
  for (nsXPTCVariant& param : mDispatchParams) {
    uint32_t arraylen = 0;
    if (!GetArraySizeFromParam(param.type, GetUndefinedHandleValue(), &arraylen)) {
      continue;
    }
...
bool CallMethodHelper::GetArraySizeFromParam(const nsXPTType& type,
  ...
  if (argnum >= mArgc && maybeArray.isObject()) {
    ...
    JS::sandbox::RootedObject arrayOrNull(mCallContext, &maybeArray.toObject());
...
bool CallMethodHelper::GetOutParamSource(uint8_t paramIndex,
  ...
  if (paramInfo.IsOut() && !isRetval) {
    ...
    if (paramIndex < mArgc) {
      JS::sandbox::RootedObject obj(mCallContext);
      if (!arg.isPrimitive()) {
        obj = &arg.toObject();
      }
...
bool CallMethodHelper::GatherAndConvertResults() {
  ...
  for (uint8_t i = 0; i < paramCount; i++) {
    ...
    JS::sandbox::RootedValue v(mCallContext, NullValue());

    uint32_t array_count = 0;
    nsID param_iid;
    if (!GetInterfaceTypeFromParam(type, &param_iid) ||
        !GetArraySizeFromParam(type, GetUndefinedHandleValue(), &array_count))
      return false;
    ...
    if (&paramInfo == mMethodInfo->GetRetval()) {
      ...
      JS::sandbox::RootedObject obj(mCallContext, &mArgv[i].toObject());
...
bool CallMethodHelper::QueryInterfaceFastPath() {
  ...
  JS::sandbox::RootedValue iidarg(mCallContext, mArgv[0]);
...
bool CallMethodHelper::ConvertIndependentParam(uint8_t i) {
  ...
  JS::sandbox::RootedValue src(mCallContext);
