...
bool XPCArrayHomogenizer::GetTypeForArray(JSContext* cx, HandleObject array,
  ...
  JS::sandbox::RootedObject jsobj(cx);
...
bool XPCVariant::InitializeData(JSContext* cx) {
  ...
  JS::sandbox::RootedValue val(cx, GetJSVal());
  ...
  if (val.isString()) {
    JS::sandbox::RootedString str(cx, val.toString());
    if (!str) {
      return false;
    }
  ...
  JS::sandbox::RootedObject jsobj(cx, &val.toObject());
...
bool XPCVariant::VariantDataToJS(JSContext* cx, nsIVariant* variant,
  ...
  JS::sandbox::RootedValue realVal(cx);
