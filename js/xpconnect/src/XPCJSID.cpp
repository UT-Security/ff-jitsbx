...
static JSObject* GetIDPrototype(JSContext* aCx, const JSClass* aClass) {
  ...
  if (!scope->mIDProto) {
    ...
    MC::RootedObject idProto(aCx, JS_NewPlainObject(aCx));
    MC::RootedObject iidProto(aCx,
                          JS_NewObjectWithGivenProto(aCx, nullptr, idProto));
    MC::RootedObject cidProto(aCx,
                          JS_NewObjectWithGivenProto(aCx, nullptr, idProto));
    MC::RootedId hasInstance(aCx,
...
Maybe<nsID> JSValue2ID(JSContext* aCx, HandleValue aVal) {
  ...
  MC::RootedObject obj(aCx, js::CheckedUnwrapStatic(&aVal.toObject()));
  if (!obj) {
    return Nothing();
  }
...
bool ContractID2JSValue(JSContext* aCx, JSString* aContract,
  ...
  MC::RootedObject obj(aCx, NewIDObjectHelper(aCx, &sCID_Class));
  if (!obj) {
    return false;
  }
  ...
  for (uint16_t i = 0; i < info->ConstantCount(); ++i) {
    if (JS_LinearStringEqualsAscii(name, info->Constant(i).Name())) {
      *resolvedp = true;
      ...
      MC::RootedValue constant(cx, info->Constant(i).JSValue());
