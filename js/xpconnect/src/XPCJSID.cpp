...
static const JSClass* sIID_Class() {
  ...
  static const JSClass __sIID_Class = {
...
static JSObject* GetIDPrototype(JSContext* aCx, const JSClass* aClass) {
  ...
  if (!scope->mIDProto) {
    ...
    bool ok =
        ...
        JS_DefineFunction(aCx, idProto, "equals", ID_EqualsCb.get(), 1,
                          kFlags) &&
        ...
        JS_DefineFunctionById(aCx, iidProto, hasInstance,
                              IID_HasInstanceCb.get(), 1, kNoEnum) &&
        ...
        JS_DefineFunction(aCx, cidProto, "createInstance",
                          CID_CreateInstanceCb.get(), 1, kFlags) &&
        JS_DefineFunction(aCx, cidProto, "getService", CID_GetServiceCb.get(),
                          1, kFlags) &&
        ...
        JS_DefineFunction(aCx, idProto, "toString", ID_GetNumberCb.get(), 0,
                          kFlags) &&
        JS_DefineFunction(aCx, iidProto, "toString", IID_GetNameCb.get(), 0,
                          kFlags) &&
        JS_DefineFunction(aCx, cidProto, "toString", CID_GetNameCb.get(), 0,
                          kFlags);
    if (!ok) {
      return nullptr;
    }
