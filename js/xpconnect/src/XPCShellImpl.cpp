...
#  endif
    ...
    if (location) {
      ...
      if (NS_SUCCEEDED(location->IsSymlink(&symlink)) && !symlink)
        location->Normalize();
      JS::sandbox::RootedObject locationObj(cx);
...
static JS::sandbox::PersistentRootedValue* sScriptedInterruptCallback = nullptr;

static bool XPCShellInterruptCallback(JSContext* cx) {
  MOZ_ASSERT(sScriptedInterruptCallback->initialized());
  JS::sandbox::RootedValue callback(cx, *sScriptedInterruptCallback);
...
static bool RegisterAppManifest(JSContext* cx, unsigned argc, Value* vp) {
  ...
  JS::sandbox::Rooted<JSObject*> arg1(cx, &args[0].toObject());
...
static bool ProcessUtf8Line(AutoJSAPI& jsapi, const char* buffer,
  ...
  JS::sandbox::RootedScript script(cx, JS::Compile(cx, options, srcBuf));
  if (!script) {
    return false;
  }
  if (compileOnly) {
  ...
  JS::sandbox::RootedValue result(cx);
  ...
  JS::sandbox::RootedString str(cx, JS::ToString(cx, result));
  if (!str) {
    return false;
  }
...
static bool ProcessFile(AutoJSAPI& jsapi, const char* filename, FILE* file,
  ...
  if (forceTTY) {
    ...
    JS::sandbox::RootedValue unused(cx);
    JS::CompileOptions options(cx);
...
static bool ProcessArgs(AutoJSAPI& jsapi, char** argv, int argc,
  ...
  FILE* rcfile;
  int rootPosition;
  JS::sandbox::Rooted<JSObject*> argsObj(cx);
  ...
  for (int j = 0, length = argc - rootPosition; j < length; j++) {
    JS::sandbox::RootedString str(cx, JS_NewStringCopyZ(cx, argv[rootPosition++]));
...
int XRE_XPCShellMain(int argc, char** argv, char** envp,
  ...
  {  // Start scoping nsCOMPtrs
    ...
    // reason to bother.
    sScriptedInterruptCallback = new JS::sandbox::PersistentRootedValue;
    ...
    // eval loop)
    {
      ...
      if (NS_SUCCEEDED(rv) && securityManager) {
        ...
        if (NS_FAILED(rv)) {
          ...
          gJSPrincipals = nsJSPrincipals::get(systemprincipal)->base_;
...
#  endif
      ...
      JS::sandbox::RootedObject lexicalEnv(cx, JS_GlobalLexicalEnvironment(glob));
