...
static bool DefineSubcomponentProperty(JSContext* aCx, HandleObject aGlobal,
  ...
  JS::sandbox::RootedValue subcompVal(aCx);
...
JSObject* GetUAWidgetScope(JSContext* cx, nsIPrincipal* principal) {
  JS::sandbox::RootedObject scope(cx, XPCJSRuntime::Get()->GetUAWidgetScope(cx, principal));
...
static void AddSizeOfCallback(JSContext*, void* aData, JS::Realm* aRealm,
