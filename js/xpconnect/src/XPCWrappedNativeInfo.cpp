...
bool XPCNativeMember::Resolve(XPCCallContext& ccx, XPCNativeInterface* iface,
  ...
  if (IsConstant()) {
    JS::sandbox::RootedValue resultVal(ccx);
    nsCString name;
...
already_AddRefed<XPCNativeInterface> XPCNativeInterface::NewInstance(
  ...
  for (unsigned int i = 0; i < constCount; i++) {
    JS::sandbox::RootedValue constant(cx);
    nsCString namestr;
    if (NS_FAILED(aInfo->GetConstant(i, &constant, getter_Copies(namestr)))) {
      return nullptr;
    }

    JS::sandbox::RootedString str(cx, JS_AtomizeString(cx, namestr.get()));
    if (!str) {
  ...
  JS::sandbox::RootedString str(cx, JS_AtomizeString(cx, bytes));
  if (!str) {
    return nullptr;
  }

  JS::sandbox::RootedId interfaceName(cx, PropertyKey::NonIntAtom(str));
