...
static bool Dump(JSContext* cx, unsigned argc, Value* vp) {
  ...
  JS::sandbox::RootedString str(cx, JS::ToString(cx, args[0]));
  if (!str) {
    return false;
  }
...
void mozJSModuleLoader::CreateLoaderGlobal(JSContext* aCx,
  ...
  // for
  JS::sandbox::RootedObject global(aCx);
...
JSObject* mozJSModuleLoader::GetSharedGlobal(JSContext* aCx) {
  if (!mLoaderGlobal) {
    JS::sandbox::RootedObject globalObj(aCx);
...
nsresult mozJSModuleLoader::LoadSingleModuleScript(
  ...
  JS::sandbox::RootedScript script(aCx);
...
JSObject* mozJSModuleLoader::PrepareObjectForLocation(JSContext* aCx,
  ...
  if (aRealFile) {
    if (XRE_IsParentProcess()) {
      JS::sandbox::RootedObject locationObj(aCx);
  ...
  JS::sandbox::RootedString exposedUri(
...
void mozJSModuleLoader::UnloadModules() {
  ...
  if (mLoaderGlobal) {
    ...
    JS::sandbox::RootedObject lexicalEnv(dom::RootingCx(),
...
JSScript* mozJSModuleLoader::InstantiateStencil(JSContext* aCx,
  ...
  if (aIsModule) {
    JS::sandbox::RootedObject module(aCx);
...
nsresult mozJSModuleLoader::ImportInto(const nsACString& registryLocation,
  ...
  JS::sandbox::RootedValue targetVal(cx, targetValArg);
  JS::sandbox::RootedObject targetObject(cx, nullptr);
...
nsresult mozJSModuleLoader::ExtractExports(JSContext* aCx,
  ...
  JS::sandbox::RootedObject symbolsObj(cx, &symbols.toObject());
  ...
  for (uint32_t i = 0; i < symbolCount; ++i) {
    ...
    if (!symbolHolder ||
      ...
      JS::sandbox::RootedString symbolStr(cx, symbolId.toString());
...
nsresult mozJSModuleLoader::Import(JSContext* aCx, const nsACString& aLocation,
  ...
  if (!mImports.Get(info.Key(), &mod) &&
    ...
    newEntry = MakeUnique<ModuleEntry>(JS::sandbox::RootingContext::get(aCx));
    ...
    JS::sandbox::RootedValue exception(aCx);
    {
  ...
  JS::sandbox::RootedObject exports(aCx, mod->exports);
...
nsresult mozJSModuleLoader::TryFallbackToImportESModule(
  ...
  JS::sandbox::RootedObject moduleNamespace(aCx);
  ...
  JS::sandbox::RootedObject globalProxy(aCx);
  {
    ...
    ModuleLoaderInfo info(aLocation);
    auto newEntry = MakeUnique<FallbackModuleEntry>(JS::sandbox::RootingContext::get(aCx));
  ...
  if (!aIgnoreExports) {
    JS::sandbox::RootedObject exports(aCx, moduleNamespace);
...
nsresult mozJSModuleLoader::TryCachedFallbackToImportESModule(
  ...
  JS::sandbox::RootedObject globalProxy(aCx, fallbackMod->globalProxy);
  if (!JS_WrapObject(aCx, &globalProxy)) {
    return NS_ERROR_FAILURE;
  }
  aModuleGlobal.set(globalProxy);
  ...
  if (!aIgnoreExports) {
    JS::sandbox::RootedObject exports(aCx, fallbackMod->moduleNamespace);
    if (!JS_WrapObject(aCx, &exports)) {
      return NS_ERROR_FAILURE;
    }
    aModuleExports.set(exports);
...
bool mozJSModuleLoader::DefineJSServices(JSContext* aCx,
  ...
  JS::sandbox::Rooted<JS::Value> services(aCx, ObjectValue(*mServicesObj));
  if (!JS_WrapValue(aCx, &services)) {
    return false;
  }

  JS::sandbox::Rooted<JS::PropertyKey> servicesId(
