...
static bool Print(JSContext* cx, unsigned argc, JS::Value* vp) {
  ...
  for (unsigned i = 0; i < args.length(); i++) {
    JS::sandbox::Rooted<JSString*> str(cx, JS::ToString(cx, args[i]));
    if (!str) return false;
...
static bool Dump(JSContext* cx, unsigned argc, JS::Value* vp) {
  ...
  JS::sandbox::Rooted<JSString*> str(cx, JS::ToString(cx, args[0]));
  if (!str) return false;
  JS::UniqueChars bytes = JS_EncodeStringToUTF8(cx, str);
  if (!bytes) return false;
...
void XPCShellEnvironment::ProcessFile(JSContext* cx, const char* filename,
  ...
  if (forceTTY) {
    ...
    JS::sandbox::Rooted<JSScript*> script(cx, JS::CompileUtf8File(cx, options, file));
    if (script) {
  ...
  do {
    ...
    *bufp = '\0';
    ...
    JS::sandbox::Rooted<JSScript*> script(cx);
...
bool XPCShellEnvironment::Init() {
  ...
  JS::sandbox::Rooted<Value> privateVal(cx, PrivateValue(this));
...
bool XPCShellEnvironment::EvaluateString(const nsAString& aString,
  ...
  JS::sandbox::Rooted<JSScript*> script(cx, JS::Compile(cx, options, srcBuf));
  if (!script) {
    return false;
  }
