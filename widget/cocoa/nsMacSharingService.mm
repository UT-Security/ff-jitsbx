...
static void SetStrAttribute(JSContext* aCx, MC::Rooted<JSObject*>& aObj, const char* aKey,
                            NSString* aVal) {
  nsAutoString strVal;
  mozilla::CopyCocoaStringToXPCOMString(aVal, strVal);
  MC::Rooted<JSString*> title(aCx, JS_NewUCStringCopyZ(aCx, strVal.get()));
  MC::Rooted<JS::Value> attVal(aCx, JS::StringValue(title));
