...
static inline bool SetJSPropertyString(JSContext* aCx,
  ...
  JS::sandbox::Rooted<JSString*> str(aCx, JS_NewStringCopyZ(aCx, aString));
  if (!str) {
    return false;
  }

  JS::sandbox::Rooted<JS::Value> val(aCx, JS::StringValue(str));
...
nsresult GfxInfoBase::GetFeatureLog(JSContext* aCx,
                                    JS::MutableHandle<JS::Value> aOut) {
  JS::sandbox::Rooted<JSObject*> containerObj(aCx, JS_NewPlainObject(aCx));
  if (!containerObj) {
    return NS_ERROR_OUT_OF_MEMORY;
  }
  aOut.setObject(*containerObj);

  JS::sandbox::Rooted<JSObject*> featureArray(aCx, JS::NewArrayObject(aCx, 0));
  if (!featureArray) {
    return NS_ERROR_OUT_OF_MEMORY;
  }
  ...
  gfxConfig::ForEachFeature([&](const char* aName, const char* aDescription,
    ...
    JS::sandbox::Rooted<JS::Value> log(aCx);
  ...
  JS::sandbox::Rooted<JSObject*> fallbackArray(aCx, JS::NewArrayObject(aCx, 0));
  if (!fallbackArray) {
    return NS_ERROR_OUT_OF_MEMORY;
  }
...
bool GfxInfoBase::BuildFeatureStateLog(JSContext* aCx,
                                       ...
                                       JS::MutableHandle<JS::Value> aOut) {
  JS::sandbox::Rooted<JSObject*> log(aCx, JS::NewArrayObject(aCx, 0));
  if (!log) {
    return false;
  }
  aOut.setObject(*log);
...
bool GfxInfoBase::InitFeatureObject(JSContext* aCx,
  ...
  JS::sandbox::Rooted<JSString*> str(aCx, JS_NewStringCopyZ(aCx, status.get()));
  JS::sandbox::Rooted<JS::Value> val(aCx, JS::StringValue(str));
  JS_SetProperty(aCx, obj, "status", val);

  // Add the feature object to the container.
  {
    JS::sandbox::Rooted<JS::Value> val(aCx, JS::ObjectValue(*obj));
