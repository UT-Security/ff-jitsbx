...
nsresult internal_ReflectHistogramAndSamples(
  ...
  MC::Rooted<JSObject*> rarray(cx, JS::NewArrayObject(cx, 2));
  if (rarray == nullptr ||
  ...
  MC::Rooted<JSObject*> values(cx, JS_NewPlainObject(cx));
  if (values == nullptr ||
...
nsresult internal_ReflectKeyedHistogram(
  ...
  for (const auto& entry : aSnapshot) {
    ...
    MC::Rooted<JSObject*> histogramSnapshot(aCx, JS_NewPlainObject(aCx));
...
nsresult internal_JS_StoreFromObjectArgument(JSContext* cx,
  ...
  if (args.length() == 0) {
    ...
    MC::Rooted<JS::Value> storeValue(cx);
    MC::Rooted<JSObject*> argsObject(cx, &args[0].toObject());
...
bool internal_JSHistogram_Snapshot(JSContext* cx, unsigned argc,
  ...
  MC::Rooted<JSObject*> snapshot(cx, JS_NewPlainObject(cx));
  if (!snapshot) {
    return false;
  }
...
bool internal_JSKeyedHistogram_Keys(JSContext* cx, unsigned argc,
  ...
  MC::RootedVector<JS::Value> autoKeys(cx);
  if (!autoKeys.reserve(keys.Length())) {
    return false;
  }

  for (const auto& key : keys) {
    MC::Rooted<JS::Value> jsKey(cx);
  ...
  MC::Rooted<JSObject*> jsKeys(cx, JS::NewArrayObject(cx, autoKeys));
  if (!jsKeys) {
    return false;
  }
...
nsresult TelemetryHistogram::CreateHistogramSnapshots(
  ...
  for (uint32_t process = 0; process < processHistArray.length(); ++process) {
    MC::Rooted<JSObject*> processObject(aCx, JS_NewPlainObject(aCx));
    if (!processObject) {
      return NS_ERROR_FAILURE;
    }
    ...
    for (const HistogramSnapshotInfo& hData : processHistArray[process]) {
      ...
      MC::Rooted<JSObject*> hobj(aCx, JS_NewPlainObject(aCx));
      if (!hobj) {
        return NS_ERROR_FAILURE;
      }
...
nsresult TelemetryHistogram::GetKeyedHistogramSnapshots(
  ...
  for (uint32_t process = 0; process < processHistArray.length(); ++process) {
    ...
    for (const KeyedHistogramSnapshotInfo& hData : processHistArray[process]) {
      ...
      MC::Rooted<JSObject*> snapshot(aCx, JS_NewPlainObject(aCx));
      if (!snapshot) {
        return NS_ERROR_FAILURE;
      }
...
nsresult internal_ParseHistogramData(
  ...
  MC::Rooted<JS::Value> histogramData(aCx);
  ...
  MC::Rooted<JS::Value> sumValue(aCx);
  MC::Rooted<JSObject*> histogramObj(aCx, &histogramData.toObject());
...
nsresult TelemetryHistogram::DeserializeHistograms(
  ...
  for (auto& processVal : processes) {
    ...
    MC::Rooted<JS::PropertyKey> histogram(aCx);
