...
nsresult TelemetryScalar::CreateSnapshots(unsigned int aDataset,
  ...
  for (const auto& entry : scalarsToReflect) {
    ...
    JS::sandbox::Rooted<JSObject*> processObj(aCx, JS_NewPlainObject(aCx));
    ...
    for (ScalarTupleArray::size_type i = 0; i < processScalars.Length(); i++) {
      ...
      JS::sandbox::Rooted<JS::Value> scalarJsValue(aCx);
...
nsresult TelemetryScalar::CreateKeyedSnapshots(
  ...
  for (const auto& entry : scalarsToReflect) {
    ...
    for (KeyedScalarTupleArray::size_type i = 0; i < processScalars.Length();
      ...
      JS::sandbox::Rooted<JSObject*> keyedScalarObj(aCx, JS_NewPlainObject(aCx));
      ...
      for (uint32_t i = 0; i < keyProps.Length(); i++) {
        ...
        JS::sandbox::Rooted<JS::Value> keyJsValue(aCx);
        nsresult rv = nsContentUtils::XPConnect()->VariantToJS(
...
nsresult TelemetryScalar::RegisterScalars(const nsACString& aCategoryName,
  ...
  JS::sandbox::Rooted<JSObject*> obj(cx, &aScalarData.toObject());
  JS::sandbox::Rooted<JS::IdVector> scalarPropertyIds(cx, JS::IdVector(cx));
  ...
  for (size_t i = 0, n = scalarPropertyIds.length(); i < n; i++) {
    ...
    JS::sandbox::Rooted<JSObject*> scalarDef(cx, &value.toObject());
...
nsresult TelemetryScalar::DeserializePersistedScalars(
  ...
  JS::sandbox::Rooted<JSObject*> scalarDataObj(aCx, &aData.toObject());
  ...
  for (auto& processVal : processes) {
    ...
    JS::sandbox::Rooted<JS::IdVector> scalars(aCx, JS::IdVector(aCx));
    if (!JS_Enumerate(aCx, processDataObj, &scalars)) {
      JS_ClearPendingException(aCx);
      continue;
    }

    JS::sandbox::Rooted<JS::PropertyKey> scalar(aCx);
    for (auto& scalarVal : scalars) {
      ...
      JS::sandbox::Rooted<JS::Value> scalarValue(aCx);
...
nsresult TelemetryScalar::DeserializePersistedKeyedScalars(
  ...
  for (auto& processVal : processes) {
    ...
    JS::sandbox::Rooted<JSObject*> processDataObj(aCx, &processData.toObject());
    JS::sandbox::Rooted<JS::IdVector> keyedScalars(aCx, JS::IdVector(aCx));
    if (!JS_Enumerate(aCx, processDataObj, &keyedScalars)) {
      JS_ClearPendingException(aCx);
      continue;
    }

    JS::sandbox::Rooted<JS::PropertyKey> keyedScalar(aCx);
    for (auto& keyedScalarVal : keyedScalars) {
      ...
      JS::sandbox::Rooted<JS::Value> keyedScalarData(aCx);
      ...
      JS::sandbox::Rooted<JSObject*> keyedScalarDataObj(aCx,
