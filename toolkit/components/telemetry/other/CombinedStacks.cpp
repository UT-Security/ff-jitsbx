...
JSObject* CreateJSStackObject(JSContext* cx, const CombinedStacks& stacks) {
  JS::sandbox::Rooted<JSObject*> ret(cx, JS_NewPlainObject(cx));
  if (!ret) {
    return nullptr;
  }
  ...
  for (size_t moduleIndex = 0; moduleIndex < moduleCount; ++moduleIndex) {
    ...
    JS::sandbox::Rooted<JSObject*> moduleInfoArray(cx, JS::NewArrayObject(cx, 0));
    if (!moduleInfoArray) {
      return nullptr;
    }
    ...
    // Module name
    JS::sandbox::Rooted<JSString*> str(cx, JS_NewUCStringCopyZ(cx, module.mName.get()));
    ...
    JS::sandbox::Rooted<JSString*> id(cx,
  ...
  JS::sandbox::Rooted<JSObject*> reportArray(cx, JS::NewArrayObject(cx, 0));
  if (!reportArray) {
    return nullptr;
  }
  ...
  for (size_t i = 0; i < length; ++i) {
    ...
    JS::sandbox::Rooted<JSObject*> pcArray(cx, JS::NewArrayObject(cx, 0));
    if (!pcArray) {
      return nullptr;
    }
    ...
    for (size_t pcIndex = 0; pcIndex < pcCount; ++pcIndex) {
      ...
      JS::sandbox::Rooted<JSObject*> framePair(cx, JS::NewArrayObject(cx, 0));
      if (!framePair) {
        return nullptr;
      }
