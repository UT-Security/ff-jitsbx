...
static bool AddLengthLimitedStringProp(JSContext* cx,
  ...
  JS::sandbox::Rooted<JS::Value> jsval(cx);
...
static bool ContainerToJSArray(JSContext* cx, JS::MutableHandle<JSObject*> aRet,
  ...
  JS::sandbox::Rooted<JSObject*> arr(cx, JS::NewArrayObject(cx, 0));
  if (!arr) {
    return false;
  }
  ...
  for (auto&& item : aContainer) {
    JS::sandbox::Rooted<JS::Value> jsel(cx);
...
static bool SerializeModule(JSContext* aCx,
  ...
  if (aFlags & nsITelemetry::INCLUDE_PRIVATE_FIELDS_IN_LOADEVENTS) {
    JS::sandbox::Rooted<JS::Value> jsFileObj(aCx);
  ...
  if (aModule->mVersion.isSome()) {
    JS::sandbox::Rooted<JS::Value> jsModuleVersion(aCx);
  ...
  JS::sandbox::Rooted<JS::Value> jsTrustFlags(aCx);
...
bool UntrustedModulesDataSerializer::SerializeEvent(
  ...
  JS::sandbox::Rooted<JS::Value> jsProcessUptimeMS(aCx);
  ...
  if (event.mLoadDurationMS) {
    JS::sandbox::Rooted<JS::Value> jsLoadDurationMS(aCx);
  ...
  JS::sandbox::Rooted<JS::Value> jsThreadId(aCx);
  ...
  if (!effectiveThreadName.IsEmpty()) {
    JS::sandbox::Rooted<JS::Value> jsThreadName(aCx);
  ...
  JS::sandbox::Rooted<JS::Value> jsBaseAddress(aCx);
  ...
  JS::sandbox::Rooted<JS::Value> jsModuleIndex(aCx);
  ...
  JS::sandbox::Rooted<JS::Value> jsIsDependent(aCx);
  ...
  JS::sandbox::Rooted<JS::Value> jsLoadStatus(aCx);
...
nsresult UntrustedModulesDataSerializer::GetPerProcObject(
  ...
  JS::sandbox::Rooted<JS::Value> jsProcType(mCx);
  ...
  JS::sandbox::Rooted<JS::Value> jsElapsed(mCx);
  ...
  if (aData.mXULLoadDurationMS.isSome()) {
    JS::sandbox::Rooted<JS::Value> jsXulLoadDurationMS(mCx);
  ...
  JS::sandbox::Rooted<JS::Value> jsSanitizationFailures(mCx);
  ...
  JS::sandbox::Rooted<JS::Value> jsTrustTestFailures(mCx);
  ...
  JS::sandbox::Rooted<JSObject*> eventsArray(mCx);
  ...
  if (!(mFlags & nsITelemetry::EXCLUDE_STACKINFO_FROM_LOADEVENTS)) {
    JS::sandbox::Rooted<JSObject*> combinedStacksObj(
...
nsresult UntrustedModulesDataSerializer::AddLoadEvents(
  ...
  JS::sandbox::Rooted<JS::Value> eventsArrayVal(mCx);
  if (!JS_GetProperty(mCx, aPerProcObj, "events", &eventsArrayVal) ||
      !eventsArrayVal.isObject()) {
    return NS_ERROR_FAILURE;
  }

  JS::sandbox::Rooted<JSObject*> eventsArray(mCx, &eventsArrayVal.toObject());
  bool isArray;
  ...
  for (auto item : aEvents) {
    JS::sandbox::Rooted<JS::Value> jsel(mCx);
...
nsresult UntrustedModulesDataSerializer::AddSingleData(
  ...
  for (const auto& entry : aData.mModules) {
    if (!mIndexMap.WithEntryHandle(entry.GetKey(), [&](auto&& addPtr) {
          if (!addPtr) {
            ...
            JS::sandbox::Rooted<JS::Value> jsModule(mCx);
  ...
  if (mFlags & nsITelemetry::EXCLUDE_STACKINFO_FROM_LOADEVENTS) {
    JS::sandbox::Rooted<JS::Value> perProcVal(mCx);
    if (JS_GetProperty(mCx, mPerProcObjContainer, strPid.get(), &perProcVal) &&
      ...
      JS::sandbox::Rooted<JSObject*> perProcObj(mCx, &perProcVal.toObject());
      return AddLoadEvents(aData.mEvents, &perProcObj);
    }
  }

  JS::sandbox::Rooted<JSObject*> perProcObj(mCx, JS_NewPlainObject(mCx));
  if (!perProcObj) {
    return NS_ERROR_FAILURE;
  }
  ...
  JS::sandbox::Rooted<JS::Value> jsPerProcObjValue(mCx);
...
UntrustedModulesDataSerializer::UntrustedModulesDataSerializer(
  ...
  JS::sandbox::Rooted<JS::Value> jsVersion(mCx);
  ...
  JS::sandbox::Rooted<JS::Value> jsModulesArrayValue(mCx);
  ...
  JS::sandbox::Rooted<JS::Value> jsBlockedModulesArrayValue(mCx);
  ...
  JS::sandbox::Rooted<JS::Value> jsPerProcObjContainerValue(mCx);
...
nsresult UntrustedModulesDataSerializer::AddBlockedModules(
  ...
  for (const auto& blockedModule : blockedModules) {
    JS::sandbox::Rooted<JS::Value> jsBlockedModule(mCx);
