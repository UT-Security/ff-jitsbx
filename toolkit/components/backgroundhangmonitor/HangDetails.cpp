...
nsHangDetails::GetAnnotations(JSContext* MC_UNSAN(aCx),
  ...
  for (size_t i = 0; i < length; ++i) {
    ...
    MC::Rooted<JSObject*> annotationPair(aCx, JS::NewArrayObject(aCx, 2));
    if (!annotationPair) {
      return NS_ERROR_OUT_OF_MEMORY;
    }

    MC::Rooted<JSString*> key(aCx,
...
nsresult StringFrame(JSContext* aCx, MC::RootedObject& aTarget, size_t aIndex,
  ...
  MC::Rooted<JSString*> string(aCx, jsString);
  if (!string) {
    return NS_ERROR_OUT_OF_MEMORY;
  }
  ...
  for (uint32_t i = 0; i < length; ++i) {
    ...
    switch (entry.type()) {
      ...
      case HangEntry::THangEntryModOffset: {
        ...
        MC::Rooted<JSObject*> jsFrame(aCx, JS::NewArrayObject(aCx, 2));
        if (!jsFrame) {
          return NS_ERROR_OUT_OF_MEMORY;
        }
        ...
        MC::Rooted<JSString*> hex(aCx, JS_NewStringCopyZ(aCx, hexString.get()));
...
nsHangDetails::GetModules(JSContext* MC_UNSAN(aCx), JS::MutableHandle<JS::Value> aVal) {
  ...
  for (size_t i = 0; i < length; ++i) {
    ...
    MC::Rooted<JSObject*> jsModule(aCx, JS::NewArrayObject(aCx, 2));
    if (!jsModule) {
      return NS_ERROR_OUT_OF_MEMORY;
    }

    MC::Rooted<JSString*> name(
