...
FOG::SetExperimentActive(const nsACString& aExperimentId,
  ...
  if (!aExtra.isNullOrUndefined()) {
    JS::sandbox::RootedObject obj(aCx, &aExtra.toObject());
...
FOG::TestGetExperimentData(const nsACString& aExperimentId, JSContext* aCx,
  ...
  JS::sandbox::RootedObject jsExperimentDataObj(aCx, JS_NewPlainObject(aCx));
  if (NS_WARN_IF(!jsExperimentDataObj)) {
    return NS_ERROR_FAILURE;
  }

  JS::sandbox::RootedValue jsBranchStr(aCx);
  ...
  JS::sandbox::RootedObject jsExtraObj(aCx, JS_NewPlainObject(aCx));
  ...
  for (unsigned int i = 0; i < extraKeys.Length(); i++) {
    JS::sandbox::RootedValue jsValueStr(aCx);
